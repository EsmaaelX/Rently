...
function sendVerificationEmail($to, $code) {
    // DEV MODE: Bypass SMTP for .test emails
    if (str_ends_with($to, '.test')) {
        return true;
    }
    ...
    try {
        ...
        // Content
        $mail->isHTML(true);
        $mail->Subject = __('Your Rently Verification Code');
        $mail->Body    = __('Here is your verification code:') . " <b>{$code}</b>";
        $mail->AltBody = __('Here is your verification code:') . " {$code}";
...
function sendNotificationEmail($to, $subject, $bodyHTML) {
    // DEV MODE: Bypass SMTP for .test emails
    if (str_ends_with($to, '.test')) {
        return true;
    }
...
function notifyAdmins($pdo, $subject, $bodyHTML) {
    $stmt = $pdo->query("SELECT email FROM users WHERE role = 'admin'");
    $admins = $stmt->fetchAll(PDO::FETCH_COLUMN);
    foreach ($admins as $adminEmail) {
        sendNotificationEmail($adminEmail, $subject, $bodyHTML);
    }
}
?>
