...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pay'])) {
    ...
    if (!isDateAvailable($pdo, $listing_id, $start_date, $end_date)) {
        ...
        if (empty($card_name) || empty($card_number) || empty($card_exp) || empty($card_cvv)) {
            ...
            // Check if there's a pending overlap, so we put them on the waitlist
            $final_status = hasPendingOverlap($pdo, $listing_id, $start_date, $end_date) ? 'waitlist' : 'pending_admin';
            
            // Create booking
            $insert = $pdo->prepare("INSERT INTO bookings (listing_id, user_id, start_date, end_date, total_price, status) VALUES (?, ?, ?, ?, ?, ?)");
            if ($insert->execute([$listing_id, $_SESSION['user_id'], $start_date, $end_date, $total, $final_status])) {
                if ($final_status === 'waitlist') {
                    $notifMsg = 'You have joined the waitlist for "' . $listing['title'] . '". If the dates become available, your request will be promoted automatically.';
                    $pdo->prepare("INSERT INTO notifications (user_id, message, link) VALUES (?, ?, ?)")->execute([$_SESSION['user_id'], $notifMsg, 'profile.php']);
                    ...
                    $pdo->prepare("INSERT INTO notifications (user_id, message, link) VALUES (?, ?, ?)")->execute([1, $adminMsg, 'admin.php?tab=bookings']);
...
<div class="text-center" style="padding: 80px 20px;">
    ...
    <p style="color:#718096; font-size:1.1rem; margin-bottom:30px;">
        <?= $message === 'waitlist_success' 
            ? __('Because these dates are pending approval for someone else, you have been added to the waitlist. If they become available, you will be notified.')
            : __('Your booking request has been sent to the owner for approval.') 
        ?>
    </p>
