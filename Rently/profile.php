...
if (isset($_GET['reject_booking'])) {
    ...
    $check = $pdo->prepare("SELECT b.id, b.user_id as renter_id, l.title, b.listing_id FROM bookings b JOIN listings l ON b.listing_id = l.id WHERE b.id = ? AND l.user_id = ?");
    $check->execute([$bid, $user_id]);
    $bData = $check->fetch();
    if ($bData) {
        ...
        promoteWaitlistedBookings($pdo, $bData['listing_id']);
    }
    redirect('profile.php');
}
...
if (isset($_GET['cancel_booking'])) {
    ...
    $bCheck = $pdo->prepare("SELECT created_at, status, listing_id FROM bookings WHERE id = ? AND user_id = ? AND status != 'rejected'");
    $bCheck->execute([$bid, $user_id]);
    $bInfo = $bCheck->fetch();
    
    if ($bInfo) {
        $age = time() - strtotime($bInfo['created_at']);
        if (in_array($bInfo['status'], ['pending_admin', 'pending_owner', 'waitlist']) || $age <= 7200) {
            $pdo->prepare("DELETE FROM bookings WHERE id = ? AND user_id = ?")->execute([$bid, $user_id]);
            promoteWaitlistedBookings($pdo, $bInfo['listing_id']);
        }
    }
    redirect('profile.php');
}
...
$stmt = $pdo->prepare("
    SELECT b.*, l.title, l.city, l.price, l.image, o.name as owner_name, o.email as owner_email, o.phone as owner_phone
    FROM bookings b 
...
// Get booking requests on MY listings (for owner to approve/reject or view approved)
$stmt = $pdo->prepare("
    SELECT b.*, l.title as listing_title, l.image as listing_image, u.name as renter_name, u.email as renter_email, u.phone as renter_phone
    FROM bookings b 
    JOIN listings l ON b.listing_id = l.id 
    JOIN users u ON b.user_id = u.id
    WHERE l.user_id = ? AND b.status IN ('pending_owner', 'approved')
    ORDER BY b.created_at DESC
");
$stmt->execute([$user_id]);
...
<div class="container" style="margin-bottom: 60px;">
    ...
    <div style="margin-bottom:40px;">
        <h3 style="margin-bottom:20px;">🔔 <?= __('Bookings on My Listings') ?> <span class="badge"><?= count($booking_requests) ?></span></h3>
        <?php foreach($booking_requests as $req): ?>
            <div style="background:var(--card-bg); padding:20px; border-radius:12px; border:1px solid var(--border-color); margin-bottom:15px; display:flex; gap:20px; align-items:center; flex-wrap:wrap;">
                <a href="view_listing.php?id=<?= $req['listing_id'] ?>"><img src="<?= htmlspecialchars($req['listing_image']) ?>" style="width:80px; height:60px; border-radius:8px; object-fit:cover;" loading="lazy"></a>
                <div style="flex:1; min-width:200px;">
                    <a href="view_listing.php?id=<?= $req['listing_id'] ?>" style="color:inherit; text-decoration:none;"><strong><?= htmlspecialchars($req['listing_title']) ?></strong></a>
                    <p style="color:#718096; font-size:14px;">👤 <?= htmlspecialchars($req['renter_name']) ?>
                    <?php if($req['status'] === 'approved'): ?>
                        (<?= htmlspecialchars($req['renter_email']) ?><?= $req['renter_phone'] ? ' - '.htmlspecialchars($req['renter_phone']) : '' ?>)
                    <?php else: ?>
                        <span style="font-style:italic; font-size:12px;">(<?= __('Contact info hidden until approved') ?>)</span>
                    <?php endif; ?>
                    </p>
                ...
                <div>
                    <?php if($req['status'] === 'pending_owner'): ?>
        ...
        <div class="grid" style="margin-bottom: 40px;">
            <?php foreach($my_bookings as $b): ?>
                <div class="card" style="flex-direction:row; height:auto; min-height:120px;">
                    <a href="view_listing.php?id=<?= $b['listing_id'] ?>"><img src="<?= htmlspecialchars($b['image']) ?>" alt="<?= htmlspecialchars($b['title']) ?>" style="width:130px; height:100%; object-fit:cover; border-radius:16px 0 0 16px;" loading="lazy"></a>
                    <div class="card-body" style="padding:15px;">
                        <h4 style="margin-bottom:5px;"><a href="view_listing.php?id=<?= $b['listing_id'] ?>" style="color:inherit; text-decoration:none;"><?= htmlspecialchars($b['title']) ?></a></h4>
                        ...
                        <?php if($b['status'] === 'approved'): ?>
                            <p style="font-size:13px; color:var(--primary-color); margin-bottom:10px;">
                                📞 <?= htmlspecialchars($b['owner_name']) ?> (<?= htmlspecialchars($b['owner_email']) ?><?= $b['owner_phone'] ? ' - '.htmlspecialchars($b['owner_phone']) : '' ?>)
                            </p>
                        <?php endif; ?>
                        <?php if($b['status'] === 'approved'): ?>
                            <span style="background:var(--success-color); color:white; padding:4px 10px; border-radius:20px; font-size:12px;"><?= __('Approved') ?></span>
                        ...
                        <?php elseif($b['status'] === 'waitlist'): ?>
                            <span style="background:#ecc94b; color:black; padding:4px 10px; border-radius:20px; font-size:12px;"><?= __('Waitlist') ?></span>
                            <a href="profile.php?cancel_booking=<?= $b['id'] ?>" style="font-size:13px; color:var(--error-color); margin-left:10px;" onclick="return confirm('Cancel this waitlist booking?');"><?= __('Cancel') ?></a>
                        <?php else: ?>
