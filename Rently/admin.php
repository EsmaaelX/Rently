...
$total_favorites = $pdo->query("SELECT COUNT(*) FROM favorites")->fetchColumn();
...
$popular_listings = $pdo->query("SELECT l.id, l.title, l.image, COUNT(b.id) as booking_count FROM bookings b JOIN listings l ON b.listing_id = l.id WHERE b.status = 'approved' GROUP BY l.id ORDER BY booking_count DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
$top_rated_owners = $pdo->query("SELECT u.id, u.name, u.profile_picture, AVG(r.rating) as avg_rating, COUNT(r.id) as review_count FROM reviews r JOIN listings l ON r.listing_id = l.id JOIN users u ON l.user_id = u.id GROUP BY u.id HAVING review_count >= 1 ORDER BY avg_rating DESC, review_count DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container" style="margin-bottom: 60px;">
    ...
    <div style="display:flex; gap:10px; margin-bottom:25px; flex-wrap:wrap;">
        <a href="admin.php?tab=analytics" class="btn <?= $tab === 'analytics' ? 'btn-primary' : '' ?>" style="<?= $tab !== 'analytics' ? 'background:var(--card-bg); color:var(--text-color); border:1px solid var(--border-color);' : '' ?>">📊 <?= __('Analytics') ?></a>
