    ...
    elseif (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if ($listing['user_id'] == $current_user) {
            ...
            if ($insert->execute([$id, $current_user, $reason])) {
                ...
                notifyAdmins($pdo, "Listing Reported: {$listing['title']}", "The listing '<strong>{$listing['title']}</strong>' has been reported.<br><br><strong>Reason:</strong> " . htmlspecialchars($reason) . "<br><br>Please review this in the admin dashboard.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_appeal'])) {
    ...
    elseif (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if ($stmt_ticket->execute([$current_user, $subject])) {
            ...
            notifyAdmins($pdo, "Report Appeal Ticket #{$ticket_id}", "The owner of '<strong>{$listing['title']}</strong>' has appealed the reports against their listing.<br><br><strong>Reason:</strong> " . htmlspecialchars($reason) . "<br><br>Please check the admin dashboard to reply.");
        ...
        <div class="listing-sidebar">
            <div style="background:var(--card-bg); padding:30px; border-radius:16px; border:1px solid var(--border-color); box-shadow:var(--shadow-hover);">
                <div style="margin-bottom:20px; border-bottom:1px solid var(--border-color); padding-bottom:15px;">
                    ...
                    <span class="badge" style="margin-top:10px;"><?= __($listing['category']) ?></span>
                </div>
            ...
            ?>
                <a href="<?= BASE_URL ?>listings/view_listing.php?id=<?= $s['id'] ?>" class="card animate-fade-in" style="text-decoration:none; color:inherit;">
                    <div class="card-img-wrapper" style="position:relative;">
                        ...
                        <span class="badge" style="position:absolute; top:15px; left:15px;"><?= __($s['category']) ?></span>
                    </div>
                    <div class="card-body">
