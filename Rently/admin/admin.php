    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if (isset($_POST['action'])) {
            ...
            if ($action === 'approve_listing') {
                ...
                $stmtOwner = $pdo->prepare("SELECT l.user_id, l.title, u.email FROM listings l JOIN users u ON l.user_id = u.id WHERE l.id = ?");
                ...
                if ($lst) {
                    ...
                    sendNotificationEmail($lst['email'], "Listing Approved", "Your listing '<strong>{$lst['title']}</strong>' has been approved by the Admin and is now live!");
                }
            ...
            elseif ($action === 'delete_listing') {
                ...
                $stmt = $pdo->prepare("SELECT l.user_id, l.title, l.image, u.email FROM listings l JOIN users u ON l.user_id = u.id WHERE l.id = ?");
                $stmt->execute([$id]);
                $listing = $stmt->fetch();
                ...
                if ($hasHistory) {
                    ...
                    if ($listing) {
                        ...
                        sendNotificationEmail($listing['email'], "Listing Deactivated", "Your listing '<strong>{$listing['title']}</strong>' was deactivated by the Admin.");
                    }
                    ...
                    if ($listing) {
                        ...
                        sendNotificationEmail($listing['email'], "Listing Deleted", "Your listing '<strong>{$listing['title']}</strong>' was deleted by the Admin.");
                    }
            ...
            elseif ($action === 'approve_booking') {
                ...
                $bk = $pdo->prepare("SELECT b.user_id as renter_id, l.user_id as owner_id, l.title, u_owner.email as owner_email FROM bookings b JOIN listings l ON b.listing_id = l.id JOIN users u_owner ON l.user_id = u_owner.id WHERE b.id = ?");
                $bk->execute([$bid]);
                $bData = $bk->fetch();
                if ($bData) {
                    ...
                    sendNotificationEmail($bData['owner_email'], "Booking Requires Your Approval", "Admin has approved a booking request for your listing '<strong>{$bData['title']}</strong>'. It is now awaiting your final approval.<br><br>Please check your dashboard.");
                }
            ...
            elseif ($action === 'reject_booking') {
                ...
                $bk = $pdo->prepare("SELECT b.user_id as renter_id, l.user_id as owner_id, l.title, u_renter.email as renter_email FROM bookings b JOIN listings l ON b.listing_id = l.id JOIN users u_renter ON b.user_id = u_renter.id WHERE b.id = ?");
                $bk->execute([$bid]);
                $bData = $bk->fetch();
                if ($bData) {
                    ...
                    sendNotificationEmail($bData['renter_email'], "Booking Rejected", "Your booking request for '<strong>{$bData['title']}</strong>' was rejected by the Admin.");
                }
            ...
            elseif ($action === 'resolve_report') {
                ...
                $stmt = $pdo->prepare("SELECT u.email FROM reports r JOIN users u ON r.user_id = u.id WHERE r.id = ?");
                $stmt->execute([$rid]);
                if ($email = $stmt->fetchColumn()) {
                    sendNotificationEmail($email, "Report Resolved", "Your report has been reviewed and resolved by the admin.");
                }
            ...
            elseif ($action === 'dismiss_report') {
                ...
                $stmt->execute([$rid]);
                if ($email = $stmt->fetchColumn()) {
                    sendNotificationEmail($email, "Report Dismissed", "Your report has been reviewed and dismissed by the admin.");
                }
...
$chart_labels_cat = array_map('__', array_column($category_distribution, 'category'));
