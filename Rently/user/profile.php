    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if ($booking && $booking['owner_id'] == $user_id && $booking['status'] === 'pending_owner') {
            if ($action === 'accept') {
                if (!checkAvailability($pdo, $booking['listing_id'], $booking['start_date'], $booking['end_date'], $booking['start_time'], $booking['end_time'], $bid)) {
                    ...
                    foreach ($overlapping as $o) {
                        ...
                        if (!($o_end <= $start_ts || $o_start >= $end_ts)) {
                            ...
                            $oEmail = $pdo->query("SELECT email FROM users WHERE id = " . (int)$o['user_id'])->fetchColumn();
                            if ($oEmail) {
                                sendNotificationEmail($oEmail, "Booking Rejected", "Your booking request for '<strong>{$booking['title']}</strong>' was rejected because another request was approved for overlapping dates/times.");
                            }
                        }
                    }
                    ...
                    $rEmail = $pdo->query("SELECT email FROM users WHERE id = " . (int)$booking['user_id'])->fetchColumn();
                    if ($rEmail) {
                        sendNotificationEmail($rEmail, "Booking Approved!", "Your booking request for '<strong>{$booking['title']}</strong>' has been approved by the owner!<br><br>Check your dashboard to see contact details and prepare for your rental.");
                    }
                ...
                $rEmail = $pdo->query("SELECT email FROM users WHERE id = " . (int)$booking['user_id'])->fetchColumn();
                if ($rEmail) {
                    sendNotificationEmail($rEmail, "Booking Rejected", "Your booking request for '<strong>{$booking['title']}</strong>' was rejected by the owner.");
                }
...
// Split "My Bookings" into Active (upcoming/pending) vs Past (completed/rejected)
$active_my_bookings = [];
$past_my_bookings = [];
foreach ($my_bookings as $b) {
    $bEnd = strtotime($b['end_date'] . ($b['end_time'] ? ' ' . $b['end_time'] : ' 23:59:59'));
    $isPast = ($b['status'] === 'rejected') || ($b['status'] === 'approved' && $bEnd < time());
    if ($isPast) {
        $past_my_bookings[] = $b;
    } else {
        $active_my_bookings[] = $b;
    }
}
...
// Aggregate confirmed earnings across all of this owner's listings
$stmt = $pdo->prepare("
    SELECT COALESCE(SUM(b.total_price), 0)
    FROM bookings b
    JOIN listings l ON b.listing_id = l.id
    WHERE l.user_id = ? AND b.status = 'approved'
");
$stmt->execute([$user_id]);
...
<div class="container" style="margin-bottom: 60px; margin-top:20px;">
    ...
    <div class="listing-layout">
        ...
        <div class="listing-main">
            ...
            <!-- Tab: My Bookings -->
            <?php
            // Shared card markup for a single "My Bookings" row (used by both Active and Past groups)
            ...
            <div id="tabContent-my-bookings" class="tab-pane" style="display:none;">
                <h3 style="margin-bottom:20px;">📅 <?= __('Active Bookings') ?></h3>
                <?php if(count($active_my_bookings) > 0): ?>
                    <div style="display:flex; flex-direction:column; gap:20px; margin-bottom:35px;">
                        <?php foreach($active_my_bookings as $b): renderMyBookingCard($b); endforeach; ?>
                    </div>
                <?php else: ?>
                    <p style="color:#718096; margin-bottom:35px;"><?= __('You have no active bookings.') ?></p>
                <?php endif; ?>

                <h3 style="margin-bottom:20px;">🗄️ <?= __('Past Bookings') ?></h3>
                <?php if(count($past_my_bookings) > 0): ?>
                    <div style="display:flex; flex-direction:column; gap:20px;">
                        <?php foreach($past_my_bookings as $b): renderMyBookingCard($b); endforeach; ?>
                    </div>
                <?php else: ?>
                    <p style="color:#718096;"><?= __('You have no past bookings.') ?></p>
                <?php endif; ?>
            </div>
            ...
            <div id="tabContent-incoming-bookings" class="tab-pane" style="display:none;">
                ...
                <div style="background:var(--card-bg); padding:20px 15px; border-radius:16px; border:1px solid var(--border-color); text-align:center; box-shadow:var(--shadow-light); margin-bottom:25px; max-width:280px;">
                    <div style="font-size:1.8rem; font-weight:800; color:var(--success-color);">₪<?= number_format($total_earnings, 2) ?></div>
                    <p style="color:#64748b; font-size:14px; margin-top:8px;">💰 <?= __('Total Confirmed Earnings') ?></p>
                </div>
