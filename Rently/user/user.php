<?php
// user/user.php - Public User Profile with Contact Privacy enforcement
...
// Get user info (excluding blocked users)
...
<div class="container" style="margin-bottom: 60px; margin-top:20px;">
    ...
    <div style="background:var(--card-bg); padding:30px; border-radius:16px; border:1px solid var(--border-color); box-shadow:var(--shadow-light); margin-bottom:40px; display:flex; gap:30px; align-items:center; flex-wrap:wrap;">
        <img src="<?= htmlspecialchars(BASE_URL . (!empty($user['profile_picture']) ? $user['profile_picture'] : 'assets/img/default_avatar.png')) ?>" 
        ...
        <div style="flex:1; min-width:250px;">
            ...
            <!-- Private Contact Details (Enforced by server-side helper) -->
            <?php 
            $viewer_id = $_SESSION['user_id'] ?? 0;
            if (canViewContactDetails($viewer_id, $user['id'])): 
            ?>
                ...
                <?php if($user['phone']): ?>
                    <p>
                        📱 <a href="tel:<?= htmlspecialchars($user['phone']) ?>" style="color:var(--primary-color); text-decoration:none;"><?= htmlspecialchars($user['phone']) ?></a>
                        <a href="https://wa.me/<?= preg_replace('/\D/', '', $user['phone']) ?>" target="_blank" class="btn" style="background:#25D366; color:white; padding:4px 8px; font-size:11px; margin-left:10px; border-radius:4px; text-decoration:none; display:inline-flex; align-items:center; gap:4px;">💬 WhatsApp</a>
                    </p>
                <?php endif; ?>
            <?php else: ?>
                <div style="background:var(--bg-color); padding:15px; border-radius:8px; border:1px dashed var(--border-color); color:#718096; font-size:13px; margin-top:10px; display:inline-block;">
                    🔒 <?= __('Contact details are private and will be visible once you have an approved booking with this user.') ?>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <?php if(count($listings) > 0): ?>
        <div class="grid" style="margin-bottom: 40px;">
            <?php foreach($listings as $l): ?>
                <a href="<?= BASE_URL ?>listings/view_listing.php?id=<?= $l['id'] ?>" class="card animate-fade-in" style="text-decoration:none; color:inherit;">
                    <div class="card-img-wrapper" style="position:relative;">
                        <img src="<?= htmlspecialchars(BASE_URL . $l['image']) ?>" alt="<?= htmlspecialchars($l['title']) ?>" loading="lazy">
                        <span class="badge" style="position:absolute; top:15px; left:15px;"><?= __($l['category']) ?></span>
                    </div>
                    <div class="card-body">
